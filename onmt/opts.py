...
def model_opts(parser):
    ...
    group.add('--enc_distillation_mode', '-enc_distillation_mode', type=str,
               default=None, choices=['regular_comb','overlap','skip_middle', 'cross_comb', 'skip_comb','skip', None],help="Do which distillation on encoder")
    group.add('--dec_distillation_mode', '-dec_distillation_mode', type=str,
               default=None, choices=['regular_comb','overlap','skip_middle', 'cross_comb', 'skip_comb','skip', None],
               help="Do which distillation on decoder")
    group.add('--logit_distillation', '-logit_distillation', action="store_true",
               help="Whether to do the distillation on the last layer or not")
    group.add('--attn_distillation_mode', '-attn_distillation_mode', type=str,
               default=None, choices=['only_self','only_context','both_self_context', None],
               help="Do which distillation on attention matrix")
    group.add('--temperature', '-temperature', type=float, default=1.0)
    group.add('--alpha', '-alpha', type=float, default=0.0, 
                help='the hyper-parameter that controls the weight of soft_loss')
    group.add('--beta', '-beta', type=float, default=0.0, 
                help='the hyper-parameter that controls the weight of internal_loss')
    group.add('--theta', '-theta', type=float, default=0.0, 
                help='the hyper-parameter that controls the weight of attn_loss')  
    group.add('--gamma', '-gamma', type=float, default=1.0, 
                help='controls weight of embedding loss')
    group.add('--kd_async', '-kd_async', action="store_true",
               help="Run Teacher model in asynchronous")
    group.add('--teacher', '-teacher', default='', type=str,
               help="If KD is applied then this is the "
                    "local path to the teacher model's state_dict.")
...
def preprocess_opts(parser):
    ...
    group.add('--data_type', '-data_type', default="text",
              help="Type of the source input. "
                   "Options are [text|img|audio|vec].")
    group.add('--data_dir', '-data_dir', type=str, required=True,
              help="Path to the raw data")
    group.add('--train_src', '-train_src', type=str, required=True,
              help="the name of the training source data")
    group.add('--train_tgt', '-train_tgt', type=str, required=True,
              help="the name of the training target data")
    ...
    group.add('--valid_src', '-valid_src', type=str, 
              help="the name of the validation source data")
    group.add('--valid_tgt', '-valid_tgt', type=str,
              help="the name of the validation target data")
...
def train_opts(parser):
    ...
    group.add('--pool_factor', '-pool_factor', type=int, default=6144,
    ...
    group.add('--early_stopping', '-early_stopping', type=int, default=15,
    ...
    group.add('--log_file', '-log_file', type=str, default="",
              help="Output logs to a file under this path.")
    group.add('--val_result_file','-val_result_file', type=str, default="validation_results.yml", help="Output validation results to a file with this name.")
    group.add('--log_file_level', '-log_file_level', type=str,
              action=StoreLoggingLevelAction,
              choices=StoreLoggingLevelAction.CHOICES,
              default="0")
...
def translate_opts(parser):
    ...
    group.add('--model_prefix', '-model_prefix', default="transformer",
              help="Model prefix used at the time of training")
    group.add('--best_n_ckpts', '-best_n_ckpts', default=1, type=int,
              help="avg best n checkpoints based upon loss")
    group.add('--train_dir', '-train_dir', required=False, default='',
              help='Path to model directory where model is stored')
    group.add('--data_dir', '-data_dir', required=False, default='',
              help='Path to data directory where data is stored')
    ...
    group.add('--val_result_file','-val_result_file', type=str, default="/cache/validation_results.yml", help="Output validation results to a file under this path.")
    group.add('--vocab_model', '-vocab_model', default="",
              help="name of vocab model file for detokenization in data_dir")
    group.add('--bleu_script', '-bleu_script', default="tools/sacre-bleu.sh",
              help="path to bleu script")
    group.add('--detok_type', '-detok_type', default="spm",
              help="detok type like spm")

    group.add('--src', '-src', type=str, default='', required=True,
              help="Source sequence to decode (one line per "
                   "sequence) multiple separated with spaces")
    group.add('--src_dir', '-src_dir', default="",
              help='Source directory for image or audio files')
    group.add('--tgt', '-tgt', type=str, default='', required=True,
