    ...
    if corpus_type == "train":
        if opt.src_vocab != "":
            try:
                ...
                existing_fields = torch.load(os.path.join(opt.data_dir, opt.src_vocab))
            except torch.serialization.pickle.UnpicklingError:
                logger.info("Building vocab from text file...")
                src_vocab, src_vocab_size = _load_vocab(
                    os.path.join(opt.data_dir, opt.src_vocab), "src", counters,
                    opt.src_words_min_frequency)
        if opt.tgt_vocab != "":
            tgt_vocab, tgt_vocab_size = _load_vocab(
                os.path.join(opt.data_dir, opt.tgt_vocab), "tgt", counters,
...
def build_save_dataset(corpus_type, fields, src_reader, tgt_reader,
    ...
    if corpus_type == 'train':
        ...
        srcs = [os.path.join(opt.data_dir, opt.train_src)]
        tgts = [os.path.join(opt.data_dir, opt.train_tgt)]
        ids = opt.train_ids
        aligns = opt.train_align
    elif corpus_type == 'valid':
        counters = None
        srcs = [os.path.join(opt.data_dir, opt.valid_src)]
        tgts = [os.path.join(opt.data_dir, opt.valid_tgt)]
        ids = [None]
...
def preprocess(opt):
    ...
    src_nfeats = 0
    tgt_nfeats = 0
    for src, tgt in zip([os.path.join(opt.data_dir, opt.train_src)], [os.path.join(opt.data_dir, opt.train_tgt)]):
    ...
    if os.path.join(opt.data_dir, opt.valid_tgt) and os.path.join(opt.data_dir, opt.valid_tgt):
