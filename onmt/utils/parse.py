...
class ArgumentParser(cfargparse.ArgumentParser):
    ...
    def validate_model_opts(cls, model_opt):
        ...
        # Distillation check
        if model_opt.logit_distillation==False:
            assert model_opt.alpha == 0.0, \
                "alpha must be zero when distillation is not applied to the last layer"
        else:
            assert model_opt.alpha != 0.0, \
                "alpha could not be zero when distillation is applied to the last layer"


        if model_opt.enc_distillation_mode is None and model_opt.dec_distillation_mode is None:
            assert model_opt.beta == 0.0, \
                "beta must be zero when internal distillation is not applied"
        else:
            assert model_opt.beta != 0.0, \
                "beta could not be zero when internal distillation is applied"
